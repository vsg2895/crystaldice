...
export default function ReviewForm({ casinoSlug }: { casinoSlug: string }) {
  ...
  if (submitted) {
    return (
      <div className="rounded-2xl border border-indigo-200 bg-indigo-50/70 p-6 text-sm text-indigo-900">
        {published ? COPY.reviews.success : COPY.reviews.successPending}
      </div>
    )
  }

  const field = 'w-full min-h-11 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition-colors placeholder:text-slate-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20'
  const label = 'mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500'
  const errorText = 'mt-1 text-xs text-rose-600'

  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_2px_18px_-10px_rgba(15,23,42,0.2)]">
      ...
      <button type="submit" disabled={loading} className="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
        {COPY.reviews.submit}
      </button>
    </form>
  )
