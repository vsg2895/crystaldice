 ...
 * component.
 *
 * Renders NOTHING when the site has reviews switched off: the API 404s, the
 * fetch helper turns that into null, and a heading with no list and no form
 * under it would be worse than absence. That one null is the whole gate — there
 * is no second features call to keep in sync with it.
 */
  ...
  return (
    <section className="mt-10" aria-labelledby="player-reviews">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        {/* `average` is null with no reviews — distinct from a rating of 0,
      ...
      {reviews.length === 0 ? (
        ...
        <ul className="mt-6 flex flex-col gap-4">
          {reviews.map((review) => (
            <li key={review.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_2px_18px_-10px_rgba(15,23,42,0.2)]">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm font-semibold text-amber-400" aria-label={`${review.rating} out of 5`}>
              ...
              <p className="mt-1 whitespace-pre-line text-sm leading-relaxed text-slate-600">{review.body}</p>
            </li>
          ))}
        </ul>
      )}
