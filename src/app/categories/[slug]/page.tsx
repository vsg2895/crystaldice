        ...
        <div className="container mx-auto max-w-5xl">
          ...
          {casinos.length > 0 && (
            <div className="mt-8 text-center">
              <Link href={`/casinos?category=${slug}`} aria-label={`See all ${category.name} casinos`} className="inline-flex rounded-full border border-slate-300 bg-white/70 px-6 py-3 text-sm font-semibold text-slate-700 backdrop-blur transition-colors hover:border-indigo-300 hover:text-indigo-700">
                See More →
              </Link>
            </div>
          )}
